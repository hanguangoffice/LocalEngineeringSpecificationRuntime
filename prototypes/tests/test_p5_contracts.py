...
from prototypes.lesr_v1 import p5_contracts
from prototypes.lesr_v1.p5_contracts import (
    PrototypeDomainService,
    RiskClass,
    WriteEnvelope,
)
from prototypes.lesr_v1.p5_mcp import create_server
...
def test_domain_contract_has_no_mcp_sdk_dependency() -> None:
    source = inspect.getsource(p5_contracts)
    assert "from mcp" not in source
    assert "import mcp" not in source


def test_resolve_query_context_and_structured_errors() -> None:
    domain = PrototypeDomainService()
    assert domain.resolve("REQ-SW-0001").payload()["value"]["uid"] == "OBJ-REQ-1"
    ...
    context = domain.build_context("coding", ("OBJ-REQ-1",), 1).payload()["value"]
...
def test_mcp_adapter_exposes_capabilities_resources_and_safe_write_schemas() -> None:
    server = create_server(PrototypeDomainService())
...
def test_real_stdio_protocol_initializes_lists_and_calls_tools() -> None:
    async def probe() -> None:
        parameters = StdioServerParameters(
            ...
            args=["-m", "prototypes.lesr_v1.p5_mcp"],
            cwd=Path.cwd(),
        )
        async with (
            ...
            assert resolved.structuredContent["value"]["uid"] == "OBJ-REQ-1"
