...
Runtime integration candidate `1.0.0a3` implements the local, single-repository, single-user
...
| F. Change/Approval | Per-object Working Copy, Candidate Revision, semantic diff, stable review subject, scoped/quorum/conditional/revocable Ed25519 approval. Three-way merge remains a domain engine and is not advertised as a public capability. | Gate 2, 4, 5 |
...
| I. Adapters/Release | Capability-filtered MCP/CLI, real LocalRuntime Web adapter and security/browser tests. Full mutation UX, Windows/Ubuntu rerun, distribution build, and external re-review remain release gates. | Gate 6, 7 |
